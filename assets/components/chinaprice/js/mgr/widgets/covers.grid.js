chinaPrice.grid.Covers = function(config) {
	...
	Ext.applyIf(config,{
		...
		,columns: [
			...
			,{header: _('chinaprice.cover_name'),dataIndex: 'name',width: 200, sortable: true}
		]
		,tbar: [{
...
chinaPrice.window.CreateCatalog = function(config) {
	...
	Ext.applyIf(config,{
		...
		,fields: [
			{xtype: 'textfield',fieldLabel: _('chinaprice.cover_name'),name: 'name',id: 'chinaprice-'+this.ident+'-name',width: 300}
		]
	});
